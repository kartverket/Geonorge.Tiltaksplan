   ...
   render() {
      ...
      return (
         ...
         <tr>
            ...
            <td data-label={this.props.translate('Measure')} style={{paddingRight : 20}} data-tip="Overordnet beskrivelse av tiltaket"><a href={measure.infoUrl} target="_blank">{measure.name}</a></td>
            <td data-label="Status" data-tip="Viser fremdrift på tiltaket" style={{paddingRight : 20}}>{this.getMeasureStatusLabel(this.props.planStatuses, measure)}</td>
            <td data-label={this.props.translate('Owner')} data-tip="Hovedansvarlig for gjennomføring av tiltaket">{measure.owner.name}</td>
            <td data-label="Sist oppdatert aktivitet" data-tip="Sist oppdatert aktivitet/rapport">{this.formatDate(measure.lastUpdatedActivity > measure.lastUpdated ? measure.lastUpdatedActivity : measure.lastUpdated)}</td>
